...
class multi30k_trainer(pl.LightningModule):
    def __init__(self, config):
        ...
        device = config_encoder['TRAIN']['DEVICE']
        ...
        self.encoder = Encoder(INPUT_DIM, config_encoder['HID_DIM'], config_encoder['N_LAYERS'],
                              config_encoder['N_HEADS'], config_encoder['PF_DIM'], config_encoder['DROPOUT'],
                              device)
        
        OUTPUT_DIM = len(multi30k.TRG.vocab)
        config_decoder = self.config['MODEL']['DECODER']
        self.decoder = Decoder(OUTPUT_DIM, config_decoder['HID_DIM'], config_decoder['N_LAYERS'],
                              config_decoder['N_HEADS'], config_decoder['PF_DIM'], config_decoder['DROPOUT'],
                              device)

        SRC_PAD_IDX = self.SRC.vocab.stoi[self.SRC.pad_token]
        TRG_PAD_IDX = self.TRG.vocab.stoi[self.TRG.pad_token]
        self.seq2seq = Seq2Seq(self.encoder, self.decoder, SRC_PAD_IDX, TRG_PAD_IDX, device, OUTPUT_DIM)
        
        self.seq2seq.to(device)
        self.seq2seq.train()
        self.seq2seq.zero_grad()
        self.seq2seq.apply(self.initialze_weights);
