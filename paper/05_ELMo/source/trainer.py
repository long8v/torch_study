...
class ELMoTrainer(pl.LightningModule):
    def __init__(self, corpus, config):
        ...
        self.petition_dl = DataLoader(
            self.petition_ds, config["DATA"]["BATCH_SIZE"], collate_fn=pad_collate
        )
    ...
    def save(self, path):
        ...
        save_yaml(
            dict(self.petition_ds.token_field.vocab.stoi_dict),
            f"{path}/token_dict.yaml",
        )
        save_yaml(
            dict(self.petition_ds.chr_field.vocab.stoi_dict), f"{path}/chr_dict.yaml"
        )
