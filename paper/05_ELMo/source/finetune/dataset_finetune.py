...
if __name__ == '__main__':
    with open('/home/long8v/torch_study/data/ynat/train_tokenized.ynat', 'r') as f:
        corpus = f.readlines()
    config = read_yaml('/home/long8v/torch_study/paper/05_ELMo/config_finetune.yaml')
    corpus = [(txt.split('\t')[1], txt.split('\t')[0]) for txt in corpus]
    pet_ds = PetitionDataset_finetune(config)
    chr_dict = read_yaml('/home/long8v/torch_study/paper/05_ELMo/source/model/elmo_30051604/chr_dict.yaml')
    token_dict = read_yaml('/home/long8v/torch_study/paper/05_ELMo/source/model/elmo_30051604/token_dict.yaml')
