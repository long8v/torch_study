...
class attention_trainer:
    def __init__(self, args):
        ...
        train_iter, valid_iter, test_iter = (
            ...
            iterators.valid_iter,
            iterators.test_iter,
        )
        self.INPUT_DIM, self.OUTPUT_DIM = (
            len(dataset.src_field.vocab),
            len(dataset.trg_field.vocab),
        )
        ...
        self.logging_train(
            ...
            self.args.clip,
            self.args.teacher_forcing_ratio,
        )
    ...
    def get_seq2seq_model(self):
        ...
        enc = Encoder(
            self.INPUT_DIM,
            self.args.ENC_EMB_DIM,
            self.args.ENC_HID_DIM,
            self.args.DEC_HID_DIM,
            self.args.ENC_DROPOUT,
        )
        dec = Decoder(
            self.OUTPUT_DIM,
            self.args.DEC_EMB_DIM,
            self.args.ENC_HID_DIM,
            self.args.DEC_HID_DIM,
            self.args.DEC_DROPOUT,
            self.args.MAXOUT_HID_DIM,
            self.args.MAXOUT_POOLSIZE,
            attn,
        )
        model = Seq2Seq(enc, dec, self.args.teacher_forcing_ratio, self.device).to(
            self.device
        )
    ...
    def logging_train(
        ...
        logging.basicConfig(
            filename=f"{save_path}/{model_name}.log", level=logging.DEBUG
        )
        ...
        for epoch in range(N_EPOCHS):
            ...
            train_loss, train_bleu = self.train(
                model, train_dl, optimizer, criterion, clip, teacher_forcing_ratio
            )
            ...
            logging.info(
                f"\tTrain Loss: {train_loss:.3f} | Train PPL: {math.exp(train_loss):7.3f} | train BLEU : {train_bleu:.3f}"
            )
            logging.info(
                f"\tVal. Loss: {valid_loss:.3f} |  Val. PPL: {math.exp(valid_loss):7.3f} | valid BLEU : {valid_bleu:.3f}"
            )
    ...
    def logging_test(self, model, test_dl, criterion):
        ...
        logging.info(
            f"| Test Loss: {test_loss:.3f} | Test PPL: {math.exp(test_loss):7.3f} | Test BLEU : {test_bleu :.3f}"
        )
