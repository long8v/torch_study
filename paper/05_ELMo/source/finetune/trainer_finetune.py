...
class gruTrainer(pl.LightningModule):
    def __init__(self, dataset, dataset_valid, finetune_config):
        ...
        self.petition_dl = DataLoader(
            self.petition_ds,
            batch_size=self.finetune_config["DATA"]["BATCH_SIZE"],
            collate_fn=pad_collate_finetune,
        )

        if dataset_valid:
            self.petition_ds = PetitionDataset_finetune(finetune_config)
            self.petition_ds_valid = self.petition_ds(
                dataset_valid, token_stoi_dict, chr_stoi_dict
            )
            self.petition_dl_valid = DataLoader(
                self.petition_ds_valid,
                batch_size=self.finetune_config["DATA"]["BATCH_SIZE"],
                collate_fn=pad_collate_finetune,
            )
        ...
        if USE_ELMO:
            self.simple_gru = simpleGRU_model_w_elmo(
            ...
            self.simple_gru = simpleGRU_model(
                self.finetune_config,
                INPUT_DIM,
                EMBEDDING_DIM,
                N_LAYERS,
        ...
        trainer = pl.Trainer(
            max_epochs=self.finetune_config["TRAIN"]["N_EPOCHS"],
...
if __name__ == "__main__":
    ...
    corpus = corpus[:1024]
    with open("~/torch_study/data/ynat/val_tokenized.ynat", "r") as f:
        corpus_valid = f.readlines()
    corpus_valid = [(txt.split("\t")[1], txt.split("\t")[0]) for txt in corpus_valid]
    corpus_valid = corpus_valid[:1024]
    config_file = "~/torch_study/paper/05_ELMo/config_finetune.yaml"
