...
class Multi30k_dataset:
    def __init__(self, src="en", trg="fr", min_freq=2):
        ...
        self.src = src
        self.trg = trg
        self.src_spacy = self.spacy_load_tokenizer(self.src)
        self.trg_spacy = self.spacy_load_tokenizer(self.trg)
        self.src_tokenizer = lambda text: self.spacy_tokenize(text, self.src_spacy)
        self.trg_tokenizer = lambda text: self.spacy_tokenize(text, self.trg_spacy)
    ...
    def get_field(self, tokenizer):
        return Field(
            tokenize=tokenizer, init_token="<sos>", eos_token="<eos>", lower=True
        )

    def load_Multi30k(self):
        # this takes so long ..
        return Multi30k.splits(
            exts=(f".{self.src}", f".{self.trg}"),
