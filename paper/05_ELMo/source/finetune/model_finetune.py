...
class simpleGRU_model_w_elmo(pl.LightningModule):
    ...
    def forward(self, token, chrs):
        ...
        self.elmo.eval()
        with torch.no_grad():
            elmo_vector = self.elmo.forward(
                chrs, finetune=True
            )  # seq_len, n_layers, n_directions, bs, hid_dim
        ...
        output = output.transpose(
            1, 0
        )  # (batch, num_layers * num_direections, hidden_size)
        output = self.fc(output[:, -1, :])  # batch_first
        return output

    def training_step(self, batch, batch_nb):
        src_token, src_chr, trg = batch
        src_toekn, src_chr, trg = (
            src_token.to(self.device),
            src_chr.to(self.device),
            trg.to(self.device),
        )
