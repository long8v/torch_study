...
class NER_Dataset(Dataset):
    def __init__(
        ...
        splitted_corpus = [[sen for sen in sentence] for sentence in splitted_corpus]
        corpus_pair = [
            [(char, bio) for char, bio in corpus]
            for corpus in splitted_corpus
            if corpus
        ]
        self.corpus_char = [
            "".join([char for char, bio in corpus]) for corpus in corpus_pair
        ]
        self.corpus_bio = [[bio for char, bio in corpus] for corpus in corpus_pair]
        self.all_labels = ["O"] + [
            f"{bio}-{cat}" for bio in ["B", "I"] for cat in categories
        ]
    ...
    def get_token_labels(self, text, label, tokenizer):
        ...
        for token_off, token in zip(offset, token_word):
            ...
            token_labels.append(
                label_clean[index : index + len_token_clean][0]
...
if __name__ == "__main__":
    ds = NER_Dataset(
        "~/torch_study/paper/file/klue-ner-v1_train.tsv",
    ...
    dl = DataLoader(
        ds, batch_size=16, collate_fn=lambda batch: pad_collate(batch, pad_idx)
    )
    for _ in dl:
        print(_.token.shape)
        print(_.label.shape)
